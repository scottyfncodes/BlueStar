...
describe("Ellen's observed baseline flows into the model", () => {
  ...
  it('DERIVES patient-facing time from the observed visit mix', () => {
    // 40 min is the result of (33% x 60) + (67% x 30), never an entered input.
    expect(weightedPatientFacingMinutes(defaultScenario())).toBeCloseTo(40, 6);
    expect(assumptionsById.get('AS-013')!.value).toBe(60);
    expect(assumptionsById.get('AS-018')!.value).toBe(30);
    expect(assumptionsById.get('AS-019')!.value!).toBeCloseTo(1 / 3, 9);
  });
  ...
  it('keeps the baseline constant separate from the editable model inputs', () => {
    ...
    expect(ELLEN_BASELINE.cycleMinutes).toBeCloseTo(55, 6);
    expect(ELLEN_BASELINE.caveat).toMatch(/not an industry productivity standard/i);
  });
});

describe('visit cycle arithmetic', () => {
  it('is 55 minutes at the baseline: 40 patient-facing + 15 travel', () => {
    const c = visitCycle(ellen);
    expect(c.patientFacingMinutes).toBeCloseTo(40, 6);
  ...
  it('counts after-hours documentation as clinician burden, not clinical capacity', () => {
    const c = visitCycle(ellen);
    expect(c.cycleMinutes).toBeCloseTo(55, 6);
    expect(c.totalClinicianMinutes).toBeCloseTo(55.5, 6);
  });
...
describe('capacity sanity check — 8 visits in a 9-to-5 day', () => {
  it('fits with room to spare: 8 x 55 = 440 of 480 minutes', () => {
    const f = scheduleFeasibility(ellen);
    expect(f.cycleMinutes).toBeCloseTo(55, 6);
  ...
  it('keeps fitting even if documentation moves to end of day', () => {
    // 60-minute cycle => 8 visits needs 480 minutes, exactly the workday.
    const f = scheduleFeasibility({ ...ellen, documentationConcurrency: 0, documentationAfterHoursShare: 0 });
    expect(f.maxVisitsPerDay).toBe(8);
    expect(f.clamped).toBe(false);
  });
...
describe('visit volume per day / week / month', () => {
  ...
  it('separates patient-facing, travel and documentation hours', () => {
    const v = capacityVolume(ellen);
    expect(v.patientFacingHoursPerWeek).toBeCloseTo((32 * 40) / 60, 6);
